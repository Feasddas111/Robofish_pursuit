...
%% 配置模型
FileName     = 'robotfish_m';               % File describing the model structure.

Order           = [6 4 6];                        % Model orders [ny nu nx].
%Parameters  = [3.0241, 3.0241, 0.0486, 2.0, 10.0, 1.0, 0.00628, 1.5, 1.5, -0.1, 0.02]';       % Initial parameter vector.
Parameters  = [3.0241, 3.0241, 0.0486, 2.0, 10.0, 1.0, 0.00319, 1.74, 2.14, -0.15, 0.04]';       % Initial parameter vector.
Parameters  = [3.0241, 3.0241, 0.0486, 2.0, 10.0, 1.0, 0.00628, 0.37, 0.46, -0.15, 0.023]';       % Initial parameter vector.
Parameters  = [3.0241, 3.0241, 0.0486, 2.0, 10.0, 1.0, 0.00369, 0.839, 1.502, -0.15, 0.0298]';       % Initial parameter vector.

InitialStates = zeros(6, 1);                % Initial states.
Ts            = 0;                                   % Time-continuous system.
nlgr = idnlgrey(FileName, Order, Parameters, InitialStates, Ts, ...
                        'Name', 'Robot Fish Depth Model',                            ...
                        'InputName',  {'Tail joint joint' 'Tail joint angular acceleration' 'Pectoral fin deflection angle' 'Pumping speed'}',            ...
                        'InputUnit', {'rad' 'rad/s^2' 'rad' 'kg/s' }',                              ...
...
%% 导入数据
dataset1 = load('tailsine.mat');
dataset2 = load('pectsine.mat');
dataset3 = load('tail-pectsine-new.mat');
...
z.InputName = {'Tail joint joint' 'Tail joint angular acceleration' 'Pectoral fin deflection angle' 'Pumping speed'};
z.InputUnit = {'rad' 'rad/s^2' 'rad' 'kg/s' };
